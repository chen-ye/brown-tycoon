var selector = '.box' // Replace this with the selector for the element you want to make transformable
...
(function() {
  ...
  makeTransformable = function(selector, callback) {
    return $(selector).each(function(i, element) {
      ...
      controlPoints = (function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          _results.push($('<div>').css({
			  ...
			  //borderRadius: 250,
            border: '300px solid rgba(250,250,250,0.1)',
            borderRadius: '300px',
            cursor: 'move',
            position: 'absolute',
            zIndex: 100000
	  ...
	  window.getPositionJson = function() {
	  	return controlPoints.map(function(p) {
	  		return [$(p).offset().left, $(p).offset().top];
	  	})
	  }
	  window.setPositionJson = function(points) {
		  // [[-86, -181], [300, 208], [1496, -216], [1402, 335]]
		  // console.log(controlPoints[2].get(0).style.left)
		  for (var i=0; i<points.length; i++) {
			  // console.log(controlPoints[i])
			  // console.log(points[i])
			  controlPoints[i].get(0).style.left = points[i][0] + 'px';
			  controlPoints[i].get(0).style.top = points[i][1] + 'px';
		  }
		  // apply new positions:
